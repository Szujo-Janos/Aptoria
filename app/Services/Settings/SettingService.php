...
class SettingService
{
    ...
    public function defaults(): array
    {
        return [
            ...
            'security.sensitive_data_detector_enabled' => ['value' => true, 'type' => 'boolean', 'group' => 'security', 'description' => 'Detect sensitive-looking data in response headers and body previews.'],
            'security.sensitive_data_max_matches' => ['value' => 30, 'type' => 'integer', 'group' => 'security', 'min' => 1, 'max' => 200, 'description' => 'Maximum sensitive data evidence matches stored per scan result.'],
            'security.unauthenticated_access_comparison_enabled' => ['value' => true, 'type' => 'boolean', 'group' => 'security', 'description' => 'Compare auth-required GET/HEAD endpoints with and without credentials.'],
            'security.unauthenticated_access_store_preview' => ['value' => true, 'type' => 'boolean', 'group' => 'security', 'description' => 'Store a masked unauthenticated response preview when access-control comparison runs.'],
            'security.schema_drift_detector_enabled' => ['value' => true, 'type' => 'boolean', 'group' => 'security', 'description' => 'Detect response JSON schema drift against the previous completed scan for the same endpoint.'],
            'security.schema_drift_max_changes' => ['value' => 50, 'type' => 'integer', 'group' => 'security', 'min' => 1, 'max' => 250, 'description' => 'Maximum schema drift changes stored per scan result.'],
            'security.schema_drift_create_findings' => ['value' => true, 'type' => 'boolean', 'group' => 'security', 'description' => 'Create a finding and evidence when response schema drift is detected.'],
